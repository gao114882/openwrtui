<template>
  <el-tabs v-if="radios.length > 0"
           :value="radios[0].name">
    <el-tab-pane v-for="radio in radios"
                 :key="radio.name"
                 :name="radio.name"
                 :label="radio.name + ` (${radio.hardware})`">
      <uci-form config="wireless"
                :apply-timeout="15">
        <uci-section :name="radio.name">
          <uci-tab :title="$t('General Settings')"
                   name="general">
            <uci-option-switch :label="$t('Disabled')"
                               name="disabled"></uci-option-switch>
            <uci-option-list :label="$t('Mode')"
                             name="hwmode"
                             :options="radio.hwmodes"
                             required></uci-option-list>
            <uci-option-list :label="$t('Band')"
                             name="htmode"
                             :options="radio.htmodes"></uci-option-list>
            <uci-option-list :label="$t('Channel')"
                             name="channel"
                             :options="radio.channels"
                             :initial="radio.channel"
                             required></uci-option-list>
            <uci-option-list :label="$t('Transmit Power')"
                             name="txpower"
                             :options="radio.txpowerlist"
                             :initial="radio.txpower"
                             required></uci-option-list>
          </uci-tab>
          <uci-tab :title="$t('Advanced Settings')"
                   name="advanced">
            <uci-option-list :label="$t('Country Code')"
                             name="country"
                             :options="radio.countrylist"
                             :initial="radio.country"
                             required></uci-option-list>
            <uci-option-input :label="$t('Distance Optimization')"
                              name="distance"
                              rules="uinteger"></uci-option-input>
          </uci-tab>
        </uci-section>
        <uci-section :title="$t('Interface')"
                     type="wifi-iface"
                     :options="{radio: radio.name}"
                     :filter="filterInterface"
                     addable
                     :after-add="afterAddIface">
          <uci-tab :title="$t('General Settings')"
                   name="general">
            <uci-option-switch :label="$t('Disabled')"
                               name="disabled"></uci-option-switch>
            <uci-option-list :label="$t('Mode')"
                             name="mode"
                             required
                             :options="modes"></uci-option-list>
            <uci-option-input label="SSID"
                              name="ssid"
                              required></uci-option-input>
            <uci-option-list :label="$t('Network')"
                             name="network"
                             :options="interfaces"></uci-option-list>
            <uci-option-switch :label="$t('Hide ESSID')"
                               name="hidden"
                               depend="mode == 'ap'"></uci-option-switch>
            <uci-option-switch :label="$t('WMM Mode')"
                               name="wmm"
                               depend="mode == 'ap'"
                               initial="1"></uci-option-switch>
          </uci-tab>
          <uci-tab :title="$t('Wireless Security')"
                   name="security">
            <uci-option-list :label="$t('Encryption')"
                             name="encryption"
                             :options="encryptions"
                             initial="none"
                             :load="loadEncr"
                             :save="saveEncr"></uci-option-list>
            <uci-option-list :label="$t('Cipher')"
                             name="cipher"
                             depend="encryption != 'none'"
                             :options="ciphers"
                             initial="auto"
                             :load="loadCipher"
                             :save="saveCipher"></uci-option-list>
            <uci-option-input :label="$t('Passphrase')"
                              name="key"
                              depend="encryption != 'none'"
                              password></uci-option-input>
          </uci-tab>
          <uci-tab :title="$t('MAC-Filter')"
                   name="macfilter">
            <uci-option-list :label="$t('Mode')"
                             name="macfilter"
                             :options="macfilters"
                             depend="mode == 'ap'"></uci-option-list>
            <uci-option-dlist :label="$t('MAC-List')"
                              name="maclist"
                              depend="macfilter == 'allow' || macfilter == 'deny'"
                              rules="macaddr"></uci-option-dlist>
          </uci-tab>
        </uci-section>
      </uci-form>
    </el-tab-pane>
