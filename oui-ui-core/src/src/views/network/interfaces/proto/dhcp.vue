<template>
  <div>
    ...
    <uci-option-switch tab="advanced"
                       :label="$t('Use broadcast flag')"
                       name="broadcast"
                       :description="$t('Required for certain ISPs, e.g. Charter with DOCSIS3')"></uci-option-switch>
    <uci-option-switch tab="advanced"
                       :label="$t('Use default gateway')"
                       name="defaultroute"
                       initial="1"
                       :description="$t('If unchecked, no default route is configured')"></uci-option-switch>
    <uci-option-switch tab="advanced"
                       :label="$t('Use DNS servers advertised by peer')"
                       name="peerdns"
                       initial="1"></uci-option-switch>
    <uci-option-dlist tab="advanced"
    ...
    <uci-option-input tab="advanced"
                      :label="$t('Client ID')"
                      name="clientid"
                      :description="$t('Client ID to send when requesting DHCP')"></uci-option-input>
    <uci-option-input tab="advanced"
                      label="Vendor Class"
                      name="vendorid"
                      :description="$t('Vendor Class to send when requesting DHCP')"></uci-option-input>
