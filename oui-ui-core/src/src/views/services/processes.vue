<template>
  <el-table :data="data"
    ...
    <el-table-column prop="cpu_percent"
                     :label="$t('CPU usage')"
                     width="100"
                     :formatter="row => row.cpu_percent + '%'"></el-table-column>
    <el-table-column prop="vsize_percent"
                     :label="$t('Memory usage')"
                     width="100"
                     :formatter="row => row.vsize_percent + '%'"></el-table-column>
    <el-table-column width="320"
                     :label="$t('Signal')">
      <template v-slot="{ row }">
        <el-button type="primary"
                   size="mini"
                   @click="kill(row.pid, 1)">{{ $t('Hang Up') }}</el-button>
        <el-button type="warning"
                   size="mini"
                   @click="kill(row.pid, 15)">{{ $t('Terminate') }}</el-button>
        <el-button type="danger"
                   size="mini"
                   @click="kill(row.pid, 9)">{{ $t('Kill immediately') }}</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
