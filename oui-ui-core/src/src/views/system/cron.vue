  ...
  <div>
    ...
    <el-table :data="data">
      ...
      <el-table-column prop="command"
                       :label="$t('Command')"></el-table-column>
      <el-table-column label="#"
                       width="200">
        <template v-slot="{$index}">
          <el-button type="primary"
                     size="mini"
                     @click="showEdit($index)">{{ $t('Edit') }}</el-button>
          <el-button type="danger"
                     size="mini"
                     @click="handleDelete($index)">{{ $t('Delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog :visible.sync="dialogVisible"
               :title="add ? $t('Add') : $t('Edit')">
      <el-form label-width="100px"
               label-position="left"
               :model="edit"
               ref="edit"
               :rules="rules">
        <el-form-item :label="$t('Minute')"
                      prop="min">
          <el-input v-model="edit.min"></el-input>
        </el-form-item>
        <el-form-item :label="$t('Hour')"
                      prop="hour">
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="handleEdit">{{ $t('OK') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
