    ...
    for (const conf in c) {
      for (const sid in c[conf])
        batch.push([
          'uci',
          'set',
          { config: conf, section: sid, values: c[conf][sid] }
        ])
      confs[conf] = true
    }
    ...
    for (const conf in n) {
      for (const sid in n[conf]) {
        ...
        for (const k in n[conf][sid]) {
          if (k === '.type') params.type = n[conf][sid][k]
          else if (k === '.create') params.name = n[conf][sid][k]
          else if (k.charAt(0) !== '.') params.values[k] = n[conf][sid][k]
        }
...
uci.apply = function(timeout) {
  ...
  return new Promise((resolve, reject) => {
    ubus
      .call('uci', 'apply', { rollback: true, timeout: timeout })
      .then(() => {
