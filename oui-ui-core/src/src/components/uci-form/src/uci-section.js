...
export default {
  ...
  computed: {
    ...
    sids() {
      ...
      if (this.filter) sections = sections.filter(s => this.filter(s, this))
  ...
  methods: {
    ...
    buildForm(sid) {
      for (const name in this.children) this.children[name].buildForm(sid)
    },
    destroyForm(sid) {
      for (const name in this.children) this.children[name].destroyFormSid(sid)
    },
    loadAdded(sid) {
    ...
    sectionView(sid, divider) {
        ...
        views.push(
          <el-row>
            <el-button
              ...
              type="danger"
              size="mini"
              on-click={this.del.bind(this, sid)}
            >
              {this.$t('Delete')}
            </el-button>
          </el-row>
        ...
        const tabPanes = this.tabs.map(tab => {
          ...
          return (
            <el-tab-pane name={tab.name}>
              <span slot="label">
                {tab.title}
                {errNum > 0 && <el-badge value={errNum} />}
              </span>
              {tab.options.map(o => (
                <uci-form-item option={o} sid={sid} />
              ))}
            </el-tab-pane>
          )
        })
      ...
      if (divider) views.push(<el-divider />)
    ...
    tableView() {
      ...
      if (this.arrayedTableExpandOptions.length > 0) {
        const scopedSlots = {
          default: props => {
            ...
            if (this.tableExpandTabs.length > 0) {
              ...
              views.push(
                <el-tabs value={this.tabs[0].name}>{tabPanes}</el-tabs>
              )
            }

            const noTabOptions = this.arrayedTableExpandOptions.filter(
              o => !o.tabName
            )
            views.push(
              noTabOptions.map(o => <uci-form-item option={o} sid={sid} />)
            )
        ...
        columns.push(
          <el-table-column type="expand" scopedSlots={scopedSlots} />
        )
      }

      columns.push(
        ...this.arrayedOptions.map(o => {
          const scopedSlots = {
            default: props => (
              <uci-form-item option={o} sid={props.row} table={true} />
            )
          }
          return (
            <el-table-column
      ...
      if (actionWidth > 0) {
            ...
            if (this.sortable) {
              actions.push(
                <el-button
                  type="success"
                  size="mini"
                  on-click={this.handleSort.bind(this, props.row, true)}
                >
                  ↑
                </el-button>
              )
              actions.push(
                <el-button
                  type="success"
                  size="mini"
                  on-click={this.handleSort.bind(this, props.row, false)}
                >
                  ↓
                </el-button>
              )
            ...
            if (this.addable && this.type && !this.name)
              actions.push(
                <el-button
                  type="danger"
                  size="mini"
                  on-click={this.del.bind(this, props.row)}
                >
                  {this.$t('Delete')}
                </el-button>
              )
        ...
        columns.push(
          <el-table-column scopedSlots={scopedSlots} width={actionWidth + 25} />
        )
      }
    ...
    collapseView() {
      const items = this.sids.map(sid => {
        ...
        if (sid !== this.activeCollapseItem) {
          ...
          if (errNum > 0) title.push(<el-badge value={errNum} />)

          const teasersValue = this.teasersValue(sid)
          const teasers = teasersValue.map((t, i) => {
            return `${t[0]}: <strong>${t[1]}</strong>${
              i < teasersValue.length - 1 ? ' | ' : ''
            }`
          })
      ...
      return (
        <el-collapse accordion v-model={this.activeCollapseItem}>
          {items}
        </el-collapse>
      )
    },
    view() {
      ...
      if (this.table) view = this.tableView()
      else if (this.collabsible && this.sids.length > 1)
        view = this.collapseView()
      else view = this.sids.map(sid => this.sectionView(sid, this.addable))
